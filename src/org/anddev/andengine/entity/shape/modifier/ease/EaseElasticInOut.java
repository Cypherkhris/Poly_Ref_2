...
public class EaseElasticInOut implements IEaseFunction, MathConstants {
	...
	public float getPercentageDone(float pSecondsElapsed, final float pDuration, final float pMinValue, final float pMaxValue) {
		...
		if(pSecondsElapsed < 1) {
			return (float) (-0.5 * (a * Math.pow(2, 10 * (pSecondsElapsed -= 1)) * ((float)Math.sin((pSecondsElapsed * pDuration - s)) * _2PI / p)) + pMinValue);
		}
		return (float) (a * Math.pow(2, -10 * (pSecondsElapsed -= 1)) * ((float)Math.sin((pSecondsElapsed * pDuration - s)) * _2PI / p) * .5 + pMaxValue + pMinValue);
	}
